...
namespace WindowsFormsApplication1
{
    public partial class Form48 : Form
    {
        ...
        {
            connection.Open();
            string query = "SELECT * FROM `catégorie` WHERE  `ID catégorie`='" + comboBox1.Text + "' ";
            MySqlCommand cmd = new MySqlCommand(query, connection);
            cmd.ExecuteNonQuery();
            MySqlDataReader rd;
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            string query1 = "UPDATE `catégorie` SET `Code catégorie`='" + textBox1.Text + "'WHERE `ID catégorie`='" + comboBox1.Text + "' ";
            connection.Open();
